...
fun HomeScreen(navController: NavHostController) {
    ...
    Column {
        Row(
            ...
            OutlinedTextField(value = textFieldID, onValueChange = {textFieldID=it}, label =
            ...
            Button(onClick = {
                if(textFieldID.trim().isEmpty()){
                    com.example.lab11sql.showAllData(studentItemsList, contextForToast)
                }else{
                    studentItemsList.clear()
                    var dbHelper = com.example.lab11sql.DatabaseHelper.getInstance(contextForToast)
                    dbHelper.writableDatabase
                    studentItemsList.clear()
                    if(Objects.isNull(dbHelper.searchStudent(textFieldID.trim()))){
                        Toast.makeText(contextForToast,"Student ID Not Found", Toast.LENGTH_SHORT).show()
                    }else{
                        studentItemsList.add(dbHelper.searchStudent(textFieldID.trim())!!)
                    }
                }
            }
            ) {
