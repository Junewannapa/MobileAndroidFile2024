...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DB_NAME,  null, DB_VERSION) {
    ...
    fun searchStudent(std_id: String): Student? {
        val db = readableDatabase
        val selectQuery = "SELECT * FROM ${com.example.lab11sql.DatabaseHelper.TABLE_NAME} WHERE ${com.example.lab11sql.DatabaseHelper.COLUMN_ID} = ?"
        var cursor: Cursor? = null
        var std: Student? = null
        try {
            cursor = db.rawQuery(selectQuery, arrayOf(std_id))
            if (cursor.moveToFirst()) {
                val id = cursor.getString(cursor.getColumnIndex(com.example.lab11sql.DatabaseHelper.COLUMN_ID))
                val name = cursor.getString(cursor.getColumnIndex(com.example.lab11sql.DatabaseHelper.COLUMN_NAME))
                val gender = cursor.getString(cursor.getColumnIndex(com.example.lab11sql.DatabaseHelper.COLUMN_GENDER))
                val age = cursor.getInt(cursor.getColumnIndex(com.example.lab11sql.DatabaseHelper.COLUMN_AGE))
